...
CREATE POLICY help_events_read ON help_search_events FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY help_events_insert ON help_search_events FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY help_clicks_read ON help_click_events FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY help_clicks_insert ON help_click_events FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY help_views_read ON help_article_views FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY help_views_insert ON help_article_views FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY help_feedback_read ON help_feedback FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY help_feedback_insert ON help_feedback FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY help_bots_read ON help_bot_sessions FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY help_bots_insert ON help_bot_sessions FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY help_zero_read ON help_zero_results FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY help_zero_insert ON help_zero_results FOR INSERT WITH CHECK (auth.role() = 'authenticated');
