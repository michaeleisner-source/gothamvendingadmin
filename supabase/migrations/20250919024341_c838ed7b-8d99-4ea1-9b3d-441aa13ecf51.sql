-- Enhanced search function using actual database columns
...
AS $function$
  ...
  where p.org_id = current_org() 
    and (p.name ilike '%'||q||'%' or coalesce(p.sku,'') ilike '%'||q||'%' or coalesce(p.category,'') ilike '%'||q||'%')
  
  union all
  -- Sales Records (search by product name, machine name, date)
  ...
  from sales s
  left join products p on p.id = s.product_id and p.org_id = current_org()
  left join machines m on m.id = s.machine_id and m.org_id = current_org()
  ...
  left join suppliers s on s.id = po.supplier_id and s.org_id = current_org()
  where po.org_id = current_org() 
    and (s.name ilike '%'||q||'%' or coalesce(po.status,'') ilike '%'||q||'%' or po.id::text ilike '%'||q||'%')
  ...
  where pr.org_id = current_org() 
    and (coalesce(pr.business_name,'') ilike '%'||q||'%' or coalesce(pr.contact_name,'') ilike '%'||q||'%' 
         or coalesce(pr.stage,'') ilike '%'||q||'%' or coalesce(pr.city,'') ilike '%'||q||'%')
  ...
  select 'supplier', sup.id, sup.name as title,
         ...
         '/suppliers' as url,
         (case when sup.name ilike '%'||q||'%' then 0.9 when coalesce(sup.contact,'') ilike '%'||q||'%' then 0.7 else 0.5 end) as rank
  from suppliers sup
  where sup.org_id = current_org() 
    and (sup.name ilike '%'||q||'%' or coalesce(sup.contact,'') ilike '%'||q||'%')
