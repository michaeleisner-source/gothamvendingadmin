-- Add missing columns to tickets table first
alter table public.tickets add column if not exists org_id uuid references public.organizations(id) on delete cascade;
alter table public.tickets add column if not exists location_id uuid references public.locations(id) on delete set null;
alter table public.tickets add column if not exists machine_id uuid references public.machines(id) on delete set null;
