-- Creates a single DRAFT PO for a chosen supplier that includes
-- the aggregated "needed" qty per product from all low-stock slots
-- (needed = restock_threshold - current_qty).
-- Assumptions:
--   - purchase_orders has: id, org_id, supplier_id, status, created_at
--   - purchase_order_items has: id, purchase_order_id, product_id, qty_ordered, unit_cost (numeric)
--   - products has cost_cents (int)
--   - suppliers table exists and you pass a valid supplier id
...
REVOKE ALL ON FUNCTION create_draft_po_for_low_stock(uuid,text) FROM public;
GRANT EXECUTE ON FUNCTION create_draft_po_for_low_stock(uuid,text) TO authenticated;