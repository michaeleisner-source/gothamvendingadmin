...
DROP TRIGGER IF EXISTS set_org_id_insurance_policies ON public.insurance_policies;
DROP TRIGGER IF EXISTS set_org_id_insurance_allocations ON public.insurance_allocations;
DROP TRIGGER IF EXISTS set_org_id_insurance_certificates ON public.insurance_certificates;
DROP TRIGGER IF EXISTS update_insurance_policies_updated_at ON public.insurance_policies;
DROP TRIGGER IF EXISTS update_insurance_allocations_updated_at ON public.insurance_allocations;
DROP TRIGGER IF EXISTS update_insurance_certificates_updated_at ON public.insurance_certificates;
...
-- Create RLS policy with DROP IF EXISTS
DROP POLICY IF EXISTS "insurance_policies_org_access" ON public.insurance_policies;
...
-- Create RLS policy
DROP POLICY IF EXISTS "insurance_allocations_org_access" ON public.insurance_allocations;
CREATE POLICY "insurance_allocations_org_access" ON public.insurance_allocations
  FOR ALL USING (is_org_member(org_id))
  WITH CHECK (org_id = current_org());
...
-- Create RLS policy
DROP POLICY IF EXISTS "insurance_certificates_org_access" ON public.insurance_certificates;
CREATE POLICY "insurance_certificates_org_access" ON public.insurance_certificates
  FOR ALL USING (is_org_member(org_id))
  WITH CHECK (org_id = current_org());
