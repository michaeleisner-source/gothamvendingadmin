...
CREATE TABLE IF NOT EXISTS help_backlog (
  ...
  q TEXT NOT NULL,
  normalized_q TEXT GENERATED ALWAYS AS (lower(unaccent(q))) STORED,
  query_hash TEXT GENERATED ALWAYS AS (md5(lower(unaccent(q)))) STORED,
  context_page TEXT,
...
CREATE POLICY IF NOT EXISTS "backlog_read" ON help_backlog 
  FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "backlog_write" ON help_backlog 
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "backlog_update" ON help_backlog 
  FOR UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "backlog_delete" ON help_backlog 
  FOR DELETE USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "escalations_read" ON help_escalations 
  FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "escalations_write" ON help_escalations 
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');
...
BEGIN
  ...
  WITH zr AS (
    SELECT lower(unaccent(q)) AS nq,
           md5(lower(unaccent(q))) AS qh,
           count(*) AS misses,
  ...
  INSERT INTO help_backlog(q, context_page, misses_90d, escalations_90d, last_seen, status, priority, category_id, notes)
  SELECT zr.nq AS q,
...
BEGIN
  ...
  slug := regexp_replace(b.normalized_q, '[^a-z0-9]+', '-', 'g');
...
CREATE OR REPLACE TRIGGER update_help_backlog_updated_at
  BEFORE UPDATE ON help_backlog
  FOR EACH ROW
