...
-- Enable RLS and create policies for ticket_sla_policies
ALTER TABLE public.ticket_sla_policies ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "ticket_sla_policies_read" ON public.ticket_sla_policies FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "ticket_sla_policies_manage" ON public.ticket_sla_policies FOR ALL USING (auth.uid() IS NOT NULL) WITH CHECK (true);
