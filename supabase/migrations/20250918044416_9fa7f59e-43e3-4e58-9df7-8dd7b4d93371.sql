-- ===== Prereqs: current_org(), is_org_member(), set_org_id() exist =====
...
-- 2) Date-range helper (last 30d default)
  ...
  po_totals as (
    select po.id,
           (select coalesce(sum(poi.qty_ordered * poi.unit_cost)::numeric,0)
              from purchase_order_items poi where poi.purchase_order_id = po.id) as total_amount
...
begin
  ...
  select
    b.user_id,
    (select pr.email from profiles pr where pr.id = b.user_id) as email, -- falls back to null if no profile
    b.orders,
    b.qty_sold,
