...
AS $function$
  ...
  select 'machine', m.id, m.name as title,
         ...
         (case when m.name ilike '%'||q||'%' then 1.0 
               when m.model ilike '%'||q||'%' then 0.8 
               when m.serial_number ilike '%'||q||'%' then 0.9 
               else 0.5 end) as rank
  from machines m
  where m.org_id = current_org() 
    and (m.name ilike '%'||q||'%' or m.model ilike '%'||q||'%' or m.serial_number ilike '%'||q||'%' or m.status ilike '%'||q||'%')
