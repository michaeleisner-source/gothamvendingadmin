...
/** Basic probe that returns status without throwing. */
async function probeSelect(table: string, columns: string) {
  try {
    const r = await (supabase as any).from(table).select(columns).limit(1);
    if (r.error) return { ok: false, code: (r.error as Any).code, message: r.error.message };
    return { ok: true, rows: r.data?.length ?? 0 };
  } catch (e:any) {
...
export default function OpsConsole() {
  ...
  /** Core: run non-destructive checks across every page's data needs. */
  async function runChecks() {
    setErrors(null); setLog([]); say("Probing tables & columns…");
    const needTables = [
      "locations","machines","products","inventory_levels",
      "delivery_routes","machine_finance",
      "machine_processor_mappings",
      "insurance_policies","insurance_allocations",
      "maintenance_schedules","machine_health_alerts"
    ];
    ...
    flags.machine_finance_monthly = await columnExists("machine_finance","monthly_payment");
  ...
  /** OPTIONAL: tiny seed (safe) for 1 product/site/machine + 1 sale. Only runs if safeMode=false. */
  async function seedTiny() {
    if (safeMode) { say("Safe Mode is ON. Toggle it off to allow writes."); return; }
    try {
      say("Seeding minimal QA data…");
      
      // Get current user's org_id from session
      const { data: { user } } = await supabase.auth.getUser();
      const orgId = user?.user_metadata?.org_id || user?.id; // fallback to user_id if no org_id
      
      // product
      const p = await supabase.from("products").select("id").eq("sku","QA-SODA-12").maybeSingle();
      let productId = p.data?.id;
      if (!productId) {
      ...
      // location
      const l = await supabase.from("locations").select("id").eq("name","QA Test Site").maybeSingle();
      let locationId = l.data?.id;
      if (!locationId) {
        const ins = await supabase.from("locations").insert({ 
          name:"QA Test Site", 
        ...
        if (ins.error) throw ins.error; 
        locationId = ins.data.id;
      }
      
      // machine
      const m = await supabase.from("machines").select("id").eq("name","QA-001").maybeSingle();
      let machineId = m.data?.id;
      if (!machineId) {
        const ins = await supabase.from("machines").insert({ 
          name:"QA-001", 
          location_id: locationId,
          org_id: orgId
        }).select("id").single();
        if (ins.error) throw ins.error; 
        machineId = ins.data.id;
      }
      
      // Note: Sales table may not exist, so we'll skip this for now
      say("✔ Seed complete (products, locations, machines created).");
      await runChecks();
    } catch (e:any) {
      setErrors(e?.message || String(e));
      say(`❌ Seed failed: ${e?.message || String(e)}`);
    }
  }
        ...
        {redRls ? (
          <div className="space-y-1 text-xs">
            ...
            <details className="mt-2">
              <summary className="cursor-pointer">Open: Dev-open policy SQL you can paste in Supabase (temporary)</summary>
    ...
    execute format('create policy dev_open on public.%I for all using (true) with check (true);', t);
      ...
      <section className="rounded-xl border border-border bg-card p-4">
        <div className="flex items-center justify-between">
          <div className="font-medium flex items-center gap-2"><Layers className="h-4 w-4" /> Minimal seed (optional)</div>
          <label className="text-xs inline-flex items-center gap-2">
            <input type="checkbox" checked={safeMode} onChange={e=>setSafeMode(e.target.checked)} />
            Safe Mode (prevent writes)
          </label>
        </div>
        <div className="mt-2 text-xs text-muted-foreground">
          Use only to drop one product/location/machine + one sale so Reports/ROI pages have data. If writes fail, check RLS above.
        </div>
        <div className="mt-2">
          <button onClick={seedTiny} className="rounded-md border border-border bg-background px-3 py-2 text-sm hover:bg-muted inline-flex items-center gap-1">
            <Activity className="h-4 w-4" /> Seed tiny QA set
          </button>
        </div>
      </section>

      <section className="rounded-xl border border-border bg-card p-4">
        <div className="font-medium mb-2 flex items-center gap-2"><FileText className="h-4 w-4" /> Log</div>
      ...
      <section className="rounded-xl border border-border bg-card p-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 text-sm">
          ...
          <Nav to="/sales" label="Sales" icon={<DollarSign className="h-4 w-4" />} />
